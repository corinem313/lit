...
if ( ! class_exists( 'Save_And_Go_Action_Next' ) ) {

	/**
	 * 'Save and Next' action: after saving the post, redirects to the
	 * edit screen of the next post (same post type).
	 */
	class Save_And_Go_Action_Next extends Save_And_Go_Action {
		...
		public function get_name() {
			return _x( 'Save and Next', 'Action name (used in settings page)', 'save-and-go' );
		}
		...
		public function get_description() {
			return _x( 'Shows the <strong>next post</strong> edit form after save.', 'Action description (used in settings page)', 'save-and-go' );
		}
		...
		public function get_button_label_pattern( $post ) {
			...
			return _x( '%s and Next', 'Button label (used in post edit page). %s = "Publish" or "Update"', 'save-and-go' );
		}

		/**
		 * Returns true only if there is a next post.
		 *
		 * @param WP_Post $post The post being edited.
		 * @return boolean
		 */
		public function is_enabled( $post ) {
			...
			return (bool) Save_And_Go_Utils::get_adjacent_post( $post, 'next' );
		}

		/**
		 * Returns the HTML title attribute for this action: the name
		 * of the next post (if there is one), else a message indicating
		...
		public function get_button_title( $post ) {
			if ( ! $this->is_enabled( $post ) ) {
				return _x( 'You are at the last post.', 'Button title attribute (used in post edit page)', 'save-and-go' );
			}

			$next_post = Save_And_Go_Utils::get_adjacent_post( $post, 'next' );

			return sprintf(
				/* translators: %s: other post name. */
				_x( 'The next post is "%s".', 'Button title attribute (used in post edit page). %s = other post name', 'save-and-go' ),
